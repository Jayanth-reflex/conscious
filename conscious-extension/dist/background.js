...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   ConsciousTabDB: () => (/* binding */ ConsciousTabDB),
...
/* harmony export */   getSessionsByDateRange: () => (/* binding */ getSessionsByDateRange),
/* harmony export */   getSettings: () => (/* binding */ getSettings),
/* harmony export */   getTodaySessions: () => (/* binding */ getTodaySessions),
/* harmony export */   initializeDefaultSettings: () => (/* binding */ initializeDefaultSettings),
/* harmony export */   logError: () => (/* binding */ logError),
/* harmony export */   logTelemetry: () => (/* binding */ logTelemetry),
/* harmony export */   popUndoBuffer: () => (/* binding */ popUndoBuffer),
/* harmony export */   purgeOldSessions: () => (/* binding */ purgeOldSessions),
/* harmony export */   pushUndoBuffer: () => (/* binding */ pushUndoBuffer),
...
// --- Dexie DB ---
class ConsciousTabDB extends dexie__WEBPACK_IMPORTED_MODULE_0__["default"] {
    constructor() {
...
const db = new ConsciousTabDB();
// --- Data Retention & Purge ---
async function purgeOldSessions(retentionDays) {
    const cutoff = Date.now() - retentionDays * 24 * 60 * 60 * 1000;
    await db.sessions.where('startTime').below(cutoff).delete();
}
// --- Backup/Restore ---
async function createBackup(encrypted = false, password) {
...
// --- Error Logging ---
async function logError(type, message, stack, sessionId) {
    await db.errors.add({
...
// --- Telemetry ---
async function logTelemetry(event, details) {
...
async function getSessions(startDate, endDate) {
    ...
    if (startDate && endDate) {
        ...
        query = query.filter(session => session.date >= startDateStr && session.date <= endDateStr);
    }
...
async function getSessionsByDateRange(days) {
...
/* harmony export */   decryptDataAES256: () => (/* binding */ decryptDataAES256),
/* harmony export */   downloadCSV: () => (/* binding */ downloadCSV),
/* harmony export */   downloadJSON: () => (/* binding */ downloadJSON),
/* harmony export */   encryptDataAES256: () => (/* binding */ encryptDataAES256),
/* harmony export */   formatTime: () => (/* binding */ formatTime),
/* harmony export */   getDomain: () => (/* binding */ getDomain),
/* harmony export */   getFocusScore: () => (/* binding */ getFocusScore),
/* harmony export */   getI18n: () => (/* binding */ getI18n),
/* harmony export */   getLocalDateString: () => (/* binding */ getLocalDateString),
/* harmony export */   getPlatformByDomain: () => (/* binding */ getPlatformByDomain),
/* harmony export */   getPlatformDomains: () => (/* binding */ getPlatformDomains),
/* harmony export */   getStreaks: () => (/* binding */ getStreaks),
/* harmony export */   getTimestamp: () => (/* binding */ getTimestamp),
/* harmony export */   getTodayDateString: () => (/* binding */ getTodayDateString),
/* harmony export */   isDomainMatch: () => (/* binding */ isDomainMatch),
/* harmony export */   isPartialDay: () => (/* binding */ isPartialDay),
/* harmony export */   isWhitelistedDomain: () => (/* binding */ isWhitelistedDomain),
/* harmony export */   parseCSVImport: () => (/* binding */ parseCSVImport),
/* harmony export */   parseJSONImport: () => (/* binding */ parseJSONImport),
...
function getPlatformDomains(platform, platformMap) {
    return platformMap[platform] || [];
}
function getPlatformByDomain(domain, platformMap) {
...
// Uses Web Crypto API
async function encryptDataAES256(data, password) {
...
async function decryptDataAES256(encrypted, password) {
    const dec = new TextDecoder();
    const { salt, iv, ciphertext } = JSON.parse(atob(encrypted));
    const keyMaterial = await getKeyMaterial(password);
    const key = await window.crypto.subtle.deriveKey({ name: 'PBKDF2', salt: new Uint8Array(salt), iterations: 100000, hash: 'SHA-256' }, keyMaterial, { name: 'AES-GCM', length: 256 }, false, ['decrypt']);
...
function parseJSONImport(jsonStr) {
    try {
...
function parseCSVImport(csvStr) {
...
function getFocusScore(activeSecs, totalSecs) {
    if (!totalSecs)
        return 0;
    return Math.round((activeSecs / totalSecs) * 100);
}
function getStreaks(sessions, minActiveSecs = 60 * 10) {
...
function isPartialDay(date, installDate) {
...
/*!***************************!*\
  ...
  \***************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _lib_db__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./lib/db */ "./src/lib/db.ts");
...
chrome.runtime.onInstalled.addListener(() => {
    ...
    (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.initializeDefaultSettings)();
...
async function handleMidnightReset() {
    ...
    // Purge old sessions
    const retention = (await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSettings)('retention')) || 90;
    await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.purgeOldSessions)(retention);
}
// --- Session Management ---
async function handleTabActivated(tab) {
    ...
    if (!url)
        return;
    const domain = (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.getDomain)(url);
    const whitelist = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSettings)('whitelist');
    if (!(0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.isWhitelistedDomain)(domain, whitelist)) {
        handleSessionEnd('switch');
        return;
    }
    // Merge session if within grace period
    const now = (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.getTimestamp)();
    ...
    // Start new session
    if (!currentSession) {
        const sessionId = (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.uuid)();
        currentSession = {
            sessionId,
            domain,
            startTime: now,
...
async function checkDailyLimit() {
    const limits = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSettings)('limits');
    const todaySessions = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getTodaySessions)();
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.action === 'undoSession') {
        (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.popUndoBuffer)().then(last => {
            if (last) {
    ...
    if (msg.action === 'logTelemetry') {
        (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.logTelemetry)(msg.event, msg.details);
    }
    if (msg.action === 'logError') {
        (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.logError)(msg.type, msg.message, msg.stack, msg.sessionId);
    }
    sendResponse({ ack: true });
});
function handleSessionEnd(reason = 'manual') {
    if (!currentSession)
        return;
    clearInterval(timer);
    clearInterval(idleCheckInterval);
    const endTime = (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.getTimestamp)();
    // Map reason to allowed interruption types
    let interruptionType = 'crash';
    ...
    (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.addSession)(sessionData);
    (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.pushUndoBuffer)(sessionData);

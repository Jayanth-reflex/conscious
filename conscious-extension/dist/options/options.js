...
async function loadSettings() {
    try {
        ...
        if (dailyLimitInput)
            dailyLimitInput.value = limits?.dailyLimit || '480';
    }
    catch (err) {
...
function setupEventListeners() {
            ...
            try {
                ...
                if (domain) {
                    ...
                    if (!whitelist.includes(domain)) {
                        ...
                        await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.setSettings)('whitelist', whitelist);
                        await loadSettings();
                    }
                }
            }
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener('click', async () => {
            try {
                ...
                let limits = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSettings)('limits');
                if (!limits)
                    limits = {};
                limits.dailyLimit = dailyLimit;
                await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.setSettings)('limits', limits);
    ...
    if (exportJSONBtn) {
        exportJSONBtn.addEventListener('click', async () => {
            try {
                const sessions = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSessionsByDateRange)(30); // Last 30 days
                (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.downloadJSON)(sessions, 'sessions.json');
            }
            catch (err) {
    ...
    if (exportCSVBtn) {
        exportCSVBtn.addEventListener('click', async () => {
            try {
                const sessions = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSessionsByDateRange)(30); // Last 30 days
                (0,_lib_utils__WEBPACK_IMPORTED_MODULE_1__.downloadCSV)(sessions, 'sessions.csv');
            }
            catch (err) {
    ...
    if (file) {
        ...
        reader.onload = async (event) => {
            try {
                ...
                const sessions = await (0,_lib_db__WEBPACK_IMPORTED_MODULE_0__.getSessionsByDateRange)(0);
                const updatedSessions = [...sessions, ...data];
                await Promise.all(updatedSessions.map(_lib_db__WEBPACK_IMPORTED_MODULE_0__.addSession));
                if (importStatus) {
